...
async def run_async(scripts: list[str], language: str) -> list[float]:
    ...
    sbx = await AsyncSandbox.create(timeout=100, request_timeout=100)
...
def curriculum_aware_reward_fn(completions, **kwargs) -> list[float]:
    ...
    new_test_cases = []
    for test_case in kwargs["verification_info"][0]["test_cases"]:
        if test_case.get("label") is not None:
            new_test_cases.append(test_case)

    kwargs["verification_info"][0]["test_cases"] = new_test_cases
    print("---------------------------")
    print("test_cases")
    print("---------------------------")
    print(kwargs["verification_info"][0]["test_cases"])
    print()
